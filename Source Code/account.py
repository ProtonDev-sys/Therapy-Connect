...
class Account():
    ...
    def signup(self, username: str, password: str, account_type: str, connection=None) -> bool:
        ...
        already_exist = connection.execute("SELECT * FROM ACCOUNT WHERE USERNAME = ?", (username,))
        ...
        connection.commit()
        account_id = connection.execute(f"SELECT * FROM ACCOUNT WHERE PASSWORD = ?", (hashed_password, ))
    ...
    def login(self, username: str, password: str) -> bool:
        ...
        cursor.execute(f"SELECT * FROM ACCOUNT WHERE USERNAME = ?", (self.username, ))
    ...
    def get_session_token(self) -> str:
        if self.session_token == None:
            ...
            connection = sqlite3.connect("database.db")
            cursor = connection.cursor()
            cursor.execute("UPDATE ACCOUNT SET SESSION_TOKEN = ? WHERE USERNAME = ?", (self.session_token, self.username))
            connection.commit()
            connection.close()
    ...
    def set_session_token(self, session_token: str) -> str:
        connection = sqlite3.connect("database.db")
        cursor = connection.cursor()
        cursor.execute(f"SELECT SESSION_TOKEN FROM ACCOUNT WHERE USERNAME = ?", (self.username,))
        result = cursor.fetchone()
        connection.close()
    ...
    def validate_session_token(self, session_token) -> bool:
        connection = sqlite3.connect("database.db")
        cursor = connection.cursor()
        cursor.execute("SELECT SESSION_TOKEN FROM ACCOUNT WHERE USERNAME = ?", (self.username,))
        result = cursor.fetchone()
        connection.close()
        if result and result[0] == session_token:
            return True
